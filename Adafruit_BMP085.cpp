...
// make a baro altitude reading and update averaged altitude
void Adafruit_BMP085::readAvgFilterAltitude(float sealevelpressure) {
	float sum = 0.0f;
	// move samples one position in buffer (oldest data in 0 position, newest in highest position)
	for (int i = 1; i < filtersize; i++) {
		sum += altsamples[i];
		altsamples[i-1] = altsamples[i];
	}
	readAltitude(sealevelpressure);
	// add new altitude measurement to buffer
	sum += altitude;
	altsamples[filtersize - 1] = altitude;
	filteredAltitude = sum / filtersize;
}
